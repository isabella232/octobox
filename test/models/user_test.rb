...
class UserTest < ActiveSupport::TestCase
  ...
  test 'sync_notifications sets job id and enqueues job in priority queue' do
    @user.sync_notifications(priority: true)
    @user.reload
    assert_not_nil @user.sync_job_id
    assert_equal 1, SyncNotificationsWorker.jobs.size
    assert_equal 'priority_sync_notifications', SyncNotificationsWorker.jobs.first["queue"]
  end
